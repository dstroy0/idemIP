...
int main(void)
{
    ...
    ROW(IDEMIP_IP4_FORWARD_BORROW);
    ROW(IDEMIP_IP6_FORWARD_BORROW);
    ROW(IDEMIP_IP4_REASS_BORROW);
    ROW(IDEMIP_IP4_FRAG_BORROW);
    ROW(IDEMIP_IGMP_BORROW);
    ROW(IDEMIP_IP6_REASS_BORROW);
    ROW(IDEMIP_IP6_FRAG_BORROW);
    ROW(IDEMIP_MLD6_BORROW);
    ROW(IDEMIP_ICMP_IN_BORROW);
    ROW(IDEMIP_ICMP6_IN_BORROW);
    ROW(IDEMIP_RAW_PCB_BORROW);
    ROW(IDEMIP_UDP_PCB_BORROW);
    ROW(IDEMIP_UDPLITE_BORROW);
    ROW(IDEMIP_TCP_PCB_BORROW);
    ROW(IDEMIP_TCP_IN_BORROW);
    ROW(IDEMIP_TCP_OUT_BORROW);
    ROW(IDEMIP_TCP_ISN_BORROW);
    ROW(IDEMIP_IP4_ADDR_BORROW);
    ROW(IDEMIP_IP6_ADDR_BORROW);
    ROW(IDEMIP_IP6_SELECT_BORROW);
    ROW(IDEMIP_PMTU4_BORROW);
    ROW(IDEMIP_DNS_BORROW);
    ROW(IDEMIP_TIMEOUTS_BORROW);
    ROW(IDEMIP_TICK_BORROW);
    ROW(IDEMIP_DISPATCH_BORROW);
