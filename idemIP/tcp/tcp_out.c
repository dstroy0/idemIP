...
static void tcp_out_build(uint8_t *restrict work)
{
    ...
    if (a->ip_version == 4u)
    {
        uint32_t src = idemip_rd32(a->local_ip);
        uint32_t dst = idemip_rd32(a->remote_ip);
        idemip_wr16(h + IDEMIP_TCP_OFF_CKSUM, idemip_tcp_cksum_compute(h, (size_t)total, src, dst));
    }
#if IDEMIP_ENABLE_IPV6
    else if (a->ip_version == 6u)
    {
        uint32_t sum = idemip_ip6_pseudo_accum(0u, a->local_ip, a->remote_ip, total, (uint8_t)IDEMIP_IP6_NH_TCP);
