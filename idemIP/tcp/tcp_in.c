...
static void tcp_in_parse(uint8_t *restrict work)
{
    ...
    if (io->parse_args.ip_version == 4u)
    {
        uint32_t src = idemip_rd32(io->parse_args.remote_ip);
        uint32_t dst = idemip_rd32(io->parse_args.local_ip);
        if (idemip_tcp_cksum_compute(h, (size_t)io->parse_args.len, src, dst) != 0u)
        {
            return;
        }
    }
#if IDEMIP_ENABLE_IPV6
    else if (io->parse_args.ip_version == 6u)
    {
        uint32_t sum = idemip_ip6_pseudo_accum(0u, io->parse_args.remote_ip, io->parse_args.local_ip,
                                               (uint32_t)io->parse_args.len, (uint8_t)IDEMIP_IP6_NH_TCP);
    ...
    {
        return; // a version that names neither pseudo-header
    }
