...
// The two versions a pseudo-header takes: RFC 791 sec 3.1 Version 4 and RFC 8200 sec 3 Version 6.
#define UDPLITE_V4 4u
#define UDPLITE_V6 6u

// RFC 791 sec 3.1 Total Length is 16 bits, so an IPv4 payload cannot pass this.
#define UDPLITE_IP4_PAYLOAD_MAX 0xFFFFu
...
// The sec 3.2 pseudo-header of one IP version, accumulated into @p sum.
//
...
//
// False when the version names no pseudo-header, or when the length that version's header cannot
// carry.
static idemip_bool udplite_pseudo(uint32_t *sum, const uint8_t *src, const uint8_t *dst, uint32_t ip_payload_len,
                                  uint8_t ip_version)
{
    if (ip_version == UDPLITE_V4)
    {
        if (ip_payload_len > (uint32_t)UDPLITE_IP4_PAYLOAD_MAX)
        {
            return IDEMIP_FALSE;
        }
        *sum = idemip_udp_pseudo_accum(0u, idemip_rd32(src), idemip_rd32(dst), (uint16_t)ip_payload_len,
                                       (uint8_t)IDEMIP_UDPLITE_PROTO);
        return IDEMIP_TRUE;
    }
#if IDEMIP_ENABLE_IPV6
    if (ip_version == UDPLITE_V6)
    {
        *sum = idemip_ip6_pseudo_accum(0u, src, dst, ip_payload_len, (uint8_t)IDEMIP_UDPLITE_PROTO);
        return IDEMIP_TRUE;
    }
#endif
    return IDEMIP_FALSE;
...
static void udplite_clear(uint8_t *restrict work)
{
    if (!work)
    {
...
// no sum is run.
static void udplite_cover(uint8_t *restrict work)
{
    if (!work)
    {
        return;
...
static void udplite_check(uint8_t *restrict work)
{
    if (!work)
    {
        return;
...
static void udplite_build(uint8_t *restrict work)
{
    if (!work)
    {
        return;
...
const UdpLiteNs UdpLite = {
    .clear = udplite_clear, .cover = udplite_cover, .check = udplite_check, .build = udplite_build};
